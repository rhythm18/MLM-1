...
$cnt=ReturnAnyValue($conn,$sql);
    ...
    if($cnt>0) 
      ...
      if($pass!=$cpass)
          ...
          {
            $cnt="select COUNT(user_id) from users";
    $memberCnt=ReturnAnyValue($conn,$cnt);
...
if($cnt>0)
{
...
else
 {
  ...
  $sql="insert into users(member_id,sponsor_id,spill_id,name,mobile,email,password,join_date) values ('$memberID','$sponsorid','$spillID','$name','$mobile','$email','$pass','$join_date')";
